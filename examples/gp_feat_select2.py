...
def opt_feat(m):
    ...
    m.optimize(max_f_eval=500, messages=False)

    # Now we resume ARD
    m.untie_everything()
    m.optimize(optimizer='lbfgsb', max_f_eval=1000, messages=False) 
...
def run_experiment(strategy, scaler, X_train, y_train, X_query,
                   y_query, output_dir, X_test, y_test=None, id_exp=1):
...
X_test = X_train[1832:]
y_test = y_train[1832:]
X_train = X_train[:1832]
y_train = y_train[:1832]
